...
func (s *SQLiteStore) AgentByFingerprint(fingerprint string) (*Agent, error) {
	...
	a := &Agent{}
	err := s.db.QueryRow(`
		SELECT a.id, a.name, a.fingerprint, a.public_key, a.bio, a.public, a.guest, a.accept_anon, a.joined_at, a.invited_by
		FROM agents a
		JOIN agent_keys ak ON a.id = ak.agent_id
		WHERE ak.fingerprint = ?`, fingerprint).Scan(&a.ID, &a.Name, &a.Fingerprint, &a.PublicKey, &a.Bio, &a.Public, &a.Guest, &a.AcceptAnon, &a.JoinedAt, &a.InvitedBy)
